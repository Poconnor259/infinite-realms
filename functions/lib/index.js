...
exports.processGameAction = (0, https_1.onCall)({ cors: true, invoker: 'public' }, async (request) => {
    ...
    try {
        ...
        const voiceResult = await (0, voice_1.generateNarrative)({
            ...
            characterProfile: currentState.character
        });
        ...
        if (voiceResult.stateReport) {
            ...
            if (voiceResult.stateReport.party) {
                const currentParty = finalState.partyMembers || [];
                ...
                if (voiceResult.stateReport.party.left) {
                    updatedParty = updatedParty.filter(member => !voiceResult.stateReport.party.left.includes(member));
                }
                finalState.partyMembers = updatedParty;
            }
            // Apply key NPCs changes
            if (voiceResult.stateReport.key_npcs) {
                const currentNpcs = finalState.keyNpcs || {};
                // Add newly met NPCs
            ...
            if (voiceResult.stateReport.quests) {
                const currentQuests = finalState.quests || {};
...
exports.keepVoiceCacheAlive = (0, https_1.onCall)(async (request) => {
    if (!request.auth)
    ...
    try {
        ...
        if (campaignData?.userId !== userId)
            throw new https_1.HttpsError('permission-denied', 'Not your campaign.');
        ...
        if (provider !== 'anthropic')
            return { success: false, message: 'Keep-alive only needed for Anthropic.' };
        // Construct Context
        let worldData = {};
        ...
        throw new https_1.HttpsError('internal', 'Keep-alive failed: ' + error.message);
    }
});
